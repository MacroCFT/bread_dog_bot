        ...
        if event.get_plaintext() == "签到":
            ...
            if player.status_code:
                ...
                if result:
                    ...
                    logger.info(
                        f"「{event.get_user_id()}」签到成功 获得{config.Currency.name}：{get_money} 签到排名：{player_sign_in_log[0]}")
                else:
                    msg = "摸摸头，今天你已经签到过了哦！"
                    result, player_sign_in_log = player.get_sign_in_log()
                    await sign_in.finish(Message(f"{MessageSegment.at(event.get_user_id())}\n签到失败！\n"
...
scheduler.add_job(reset_sign_in, "interval")
        ...
        if event.get_user_id() in admins:
            ...
            if len(text) == 3:
                ...
                if result:
                    ...
                    logger.info(
                        f"「{event.get_user_id()}」给 「{player.name}({player.qq})」添加了 {money} {config.Currency.name}")
                else:
                    await add_money.finish(Message(f"添加失败！\n{player_money}"))
            else:
                await add_money.finish(
        ...
        if event.get_user_id() in admins:
            ...
            if len(text) == 3:
                ...
                if result:
                    ...
                    logger.info(
                        f"「{event.get_user_id()}」扣除了 「{player.name}({player.qq})」 {money} {config.Currency.name}")
                else:
                    await sub_money.finish(Message(f"扣除失败！\n{player_money}"))
            else:
                await sub_money.finish(
        ...
        if event.get_user_id() in admins:
            ...
            if len(text) == 3:
                ...
                if result:
                    ...
                    logger.info(
                        f"「{event.get_user_id()}」给 「{player.name}({player.qq})」 设置了 {money} {config.Currency.name}")
                else:
                    await set_money.finish(Message(f"设置失败！\n{player_money}"))
            else:
                await set_money.finish(
